...
require_once('class/connection.php'); 
    ...
    if($category == 0){
        $sql = "SELECT c.judul, u.nama, c.idcerita, COUNT(p.idparagraf) as jumlahPar  from cerita as c INNER JOIN users as u on c.idusers_pembuat_awal = u.idusers INNER JOIN paragraf as p on p.idcerita = c.idcerita GROUP BY idcerita limit ?,?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param('ii', $startLimit, $perpage);
        $stmt->execute();
    }else{
